if (window.location.pathname.includes("admin")) {
	$(document).ready(function () {
		...
		if (document.title.includes("Kategori")) {
			kategori();
		}
		if (document.title.includes("User")) {
			user();
		}
	...
	function addUser() {
		...
		if (!$("#usernameadd").val().match(uname)) {
			$("#errorusr").text(
				"*Username min 5 karakter harus berupa huruf atau angka"
			);
			unm = "NO";
		} else {
			unm = "YES";
		}
		if (!$("#passwordadd").val().match(passw)) {
			$("#errorpwd").text(
				"*Password min 6 karakter berupa huruf besar & kecil, & angka"
			);
			pwd = "NO";
